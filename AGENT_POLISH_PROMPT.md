...
1. **Website** — `C:/github/gordongreco.com/` — Static HTML/CSS/JS marketing site
2. **Advisory Suite** — `C:/github/Markos_Analytics_Suite/Financial-Engine/` — Python PDF generators
...
## SECTION 2 — Website (`C:/github/gordongreco.com/`)

### 2.1 Current State

The site is a static HTML/CSS/JS site using Tailwind CSS (`css/tailwind.min.css`) and custom styles
(`css/style.css`). It has 4 top-level pages + 5 service sub-pages.

**Deployment:** Configured for Netlify (netlify.toml present). Domain: `gordongreco.com`.
CNAME file is set. Canonical tags are on all pages.

### 2.2 Fixes Still Required

**CRITICAL — Legal pages (create these):**

1. **`privacy.html`** — Privacy Policy. Must cover:
...
2. **`terms.html`** — Terms of Service / Disclosures page. Must cover:
...
3. **Add links to both pages in the footer** — currently missing.
   In every HTML file's footer, after the existing disclaimer block, add:
   ```html
...
**Netlify (recommended — free tier is sufficient):**
1. Connect the `gordongreco.com` GitHub repo to Netlify
2. Build command: none (static site)
3. Publish directory: `.` (root)
4. Set custom domain: `gordongreco.com` in Netlify dashboard
5. Netlify auto-provisions SSL via Let's Encrypt
6. The `netlify.toml` in the repo handles: HTTP→HTTPS redirect, www→apex redirect,
...
- Verify each PDF compiles and renders: `venv/Scripts/python.exe Financial-Engine/<generator>.py`
- Run `venv/Scripts/python.exe -m py_compile Financial-Engine/<file>.py` before running
...
2. Create `privacy.html` and `terms.html` for the website
3. Add footer links to privacy/terms on all 9 HTML pages
