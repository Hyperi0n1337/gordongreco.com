...
const client=fs.readFileSync(new URL('../public/portal/app.js',import.meta.url),'utf8');
...
test('mobile async handlers retain form references',()=>{assert.ok(client.includes('const form=event.currentTarget'));assert.ok(client.includes("form.elements.namedItem('file')"));assert.ok(!client.includes('e.currentTarget.file'));assert.ok(!client.includes('e.currentTarget.message'))});
